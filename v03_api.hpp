...
ContainerT<Tag, Studentas> read_all(const std::string& path, long long* out_read_ms = nullptr);

template<typename Tag>
ContainerT<Tag, Studentas> read_all(const std::string& path, int method, long long* out_read_ms = nullptr);
